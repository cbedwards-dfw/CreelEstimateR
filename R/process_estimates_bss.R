...
#' @return ??
#' @export
process_estimates_bss <- function(params,
                                  analysis_lut,
                                  estimates_bss) {
  ...
  transformed_bss_data$bss_stratum <- all_data |>
    ...
    dplyr::mutate(
      ...
      min_event_date = .data$event_date, #Apply BSS daily event date to min and max dates
      max_event_date = .data$event_date,
  ...
  transformed_bss_data$bss_stratum_catch <- transformed_bss_data$bss_stratum_catch |>
    ...
    dplyr::mutate(day_type = ifelse(weekdays(.data$min_event_date) %in% params$days_wkend, "weekend", "weekday"))
