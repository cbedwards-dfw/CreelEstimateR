...
prep_dwg_interview_fishing_time <- function(
  ...
  if(string::str_detect(study_design, "tandard" ) | study_design == "Drano"){ # If Study design in the YAML is specified as [S]tandard or Drano proceed with running this function...
    ...
    interview_fishing_time <-
      dwg_interview |>
      dplyr::mutate(
        trip_status = tidyr::replace_na(.data$trip_status, "Unknown"),
        end_time_final =
          dplyr::if_else(
            .data$trip_status == "Incomplete" | is.na(.data$fishing_end_time),
            .data$interview_time,
            .data$fishing_end_time
          ),
        fishing_time = round(as.numeric(.data$end_time_final - .data$fishing_start_time) / 3600, 5),
        vehicle_count = dplyr::if_else(.data$vehicle_count > .data$person_count_final,
                                .data$person_count_final,
                                .data$vehicle_count), #KB - I don't understand why this line of code is here; seems like a QAQC update but not sure
        fishing_time_total = .data$fishing_time * .data$person_count_final
      ) |>
      dplyr::filter(.data$fishing_time >= min_fishing_time) # |>
